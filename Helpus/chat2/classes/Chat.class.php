...
class Chat{
	...
	public function inserir(){
		$strSQL = "INSERT INTO chat2 SET nome = ?, mensagem = ? , datahora = NOW()";
	...
	public function existeNome(){
		$strSQL = "SELECT COUNT(nome) FROM chat2 WHERE nome = ?";
		$stmt  = BD::getConn()->prepare($strSQL);
	...
	public function excluir(){
		$strSQL = "DELETE FROM chat2 WHERE DATE_ADD(datahora, INTERVAL {$this->tempoLimite} DAY) < NOW()";
		$stmt   = BD::getConn()->query($strSQL);
	}
	public function listar(){
		$strSQL = "SELECT * FROM chat2 ORDER BY id DESC";
