...
class Chat{
	...
	public function inserir(){
		$strSQL = "INSERT INTO chat7 SET nome = ?, mensagem = ? , datahora = NOW()";
	...
	public function existeNome(){
		$strSQL = "SELECT COUNT(nome) FROM chat7 WHERE nome = ?";
		$stmt  = BD::getConn()->prepare($strSQL);
	...
	public function excluir(){
		$strSQL = "DELETE FROM chat7 WHERE DATE_ADD(datahora, INTERVAL {$this->tempoLimite} DAY) < NOW()";
		$stmt   = BD::getConn()->query($strSQL);
	}
	public function listar(){
		$strSQL = "SELECT * FROM chat7 ORDER BY id DESC";
