...
class Chat{
	...
	public function inserir(){
		$strSQL = "INSERT INTO chat10 SET nome = ?, mensagem = ? , datahora = NOW()";
	...
	public function existeNome(){
		$strSQL = "SELECT COUNT(nome) FROM chat10 WHERE nome = ?";
		$stmt  = BD::getConn()->prepare($strSQL);
	...
	public function excluir(){
		$strSQL = "DELETE FROM chat10 WHERE DATE_ADD(datahora, INTERVAL {$this->tempoLimite} DAY) < NOW()";
		$stmt   = BD::getConn()->query($strSQL);
	}
	public function listar(){
		$strSQL = "SELECT * FROM chat10 ORDER BY id DESC";
